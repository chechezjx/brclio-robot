...
- 18 个原创教学关卡和自由实验室，逐步开放顺序、步数、转向、后退、两层循环、A/B 动作组合及 Python 基础写法。所有关卡可直接选择，无倒计时或失败惩罚。
- 第 13～18 关保留拖拽积木与机器人闯关，使用 `robot.forward(3)`、`robot.backward(1)`、`robot.turn_left()`、`robot.turn_right()` 和 `for _ in range(3):` 展示指令，并同步预览缩进后的 Python 代码。从单层循环逐步进阶到复杂路线、双层循环、自定义函数和两个函数协作。
...
4. 第 10～12 关及自由实验室可在动作组合 A/B 区定义动作，再在主程序放入「执行 A/B」。第 13～18 关使用 Python 写法，括号内的数字仍可编辑，每进入一层循环或函数就多缩进 4 个空格。第 17～18 关在 `def action_a():` / `def action_b():` 区定义函数，再用 `action_a()` / `action_b()` 调用；函数定义本身不自动执行。
...
src/
  ...
  data/levels.ts        18 个教学关卡、自由练习、结构化参考解
