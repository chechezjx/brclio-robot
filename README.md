...
- 23 个原创教学关卡和自由实验室，逐步开放顺序、步数、转向、后退、两层循环、A/B 动作组合及 Python 基础写法。所有关卡可直接选择，无倒计时或失败惩罚。
- 第 13～23 关保留拖拽积木与机器人闯关，使用 `robot.forward(3)`、`robot.backward(1)`、`robot.turn_left()`、`robot.turn_right()` 和 `for _ in range(3):` 展示指令，并同步预览缩进后的 Python 代码。从单层循环逐步进阶到复杂路线、双层循环、自定义函数和两个函数协作。
...
4. 第 10～12 关及自由实验室可在动作组合 A/B 区定义动作，再在主程序放入「执行 A/B」。第 13～23 关使用 Python 写法，括号内的数字仍可编辑，每进入一层循环或函数就多缩进 4 个空格。第 17～23 关在 `def action_a():` / `def action_b():` 区定义函数，再用 `action_a()` / `action_b()` 调用；函数定义本身不自动执行。函数也可以调用另一个函数，但不能直接或间接调用自己。
...
第 14～18 关承接第 13 关，每关保留“收齐星星、程序结束时停在旗子上”的规则，并通过地图、积木数量提示和三级提示逐步增加挑战。

| 关卡 | 名称 | 新增挑战 |
| --- | --- | --- |
| 14 | 循环前后的小任务 | 在循环前后安排行动，先后退收星，再完成最后一段路线 |
| 15 | 穿越星光拱门 | 在一次循环中组合多次转向和不同步数，连续绕过障碍 |
| 16 | 双层循环星环 | 用内层循环搭建台阶，外层循环改变方向，走完星环 |
| 17 | 函数导航小队 | 用 `def action_a():` 封装循环路线，在不同位置调用 |
| 18 | Python 星际总动员 | 让 `action_a()`、`action_b()` 配合循环和连接路线完成综合挑战 |

第 19～23 关继续增加路线拆分和组合的难度，每关仍提供三级提示。函数之间采用单向调用，避免递归。

| 关卡 | 名称 | 新增挑战 |
| --- | --- | --- |
| 19 | 函数接力航线 | 在 B 中调用 A，并在循环内外复用整段 B 路线 |
| 20 | 支路探测小队 | 将前进、后退和转向封装成往返函数，返回原位置和原朝向 |
| 21 | 双层巡检站 | 用两层循环组织巡检点和边，再补查额外站点 |
| 22 | 三区往返航线 | 在不同位置和朝向复用往返函数，规划三个区域的连接路线 |
| 23 | 星际调度总挑战 | 综合往返探测、双层循环、函数调用和分区衔接 |
...
Python 关卡通过 `Level.syntax: 'python'` 选择指令展示方式，程序依然保存为相同的版本 1 积木数据并由机器人解释器执行。Python 预览用于学习调用、参数、循环与缩进；`robot` 是本游戏的教学接口，不是 Python 内置对象，应用不提供任意 Python 文本执行环境。原有关卡草稿、进度和 JSON 导入导出格式保持兼容。
...
4. 将关卡加入 `levels`（自由地图放入相应集合），导航、进度和章节分组按关卡数据生成。需要展示 Python 写法时设置 `syntax: 'python'`；省略时保留中文积木。
